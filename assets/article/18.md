这篇文章分析了Pagination组件，还是比较好理解的，下面直接上代码。
...
export interface PaginationProps {
  total?: number;  // 总共的页数
  defaultCurrent?: number; // 当前的默认是第几页
  current?: number;  // 当前是第几页。如果既有默认当前又有当前，那么当前的优先级高于默认当前
  defaultPageSize?: number;  // 默认每页的条数
  pageSize?: number;  // 每页的条数，pageSize的优先级要比defaultPageSize的优先级要高
  onChange?: (page: number, pageSize?: number) => void;  // 页面改变时的回调函数
  hideOnSinglePage?: boolean;  // 只有一页时是否隐藏分页
  showSizeChanger?: boolean;  // 是否可以改变每页的条数
  pageSizeOptions?: string[];  // 每页显示条数的可供选择的项目
  onShowSizeChange?: (current: number, size: number) => void;  // 每页显示条数改变时的回调函数
  showQuickJumper?: boolean;  // 是否可以快速跳转至某页
  showTotal?: (total: number, range: [number, number]) => React.ReactNode;  // 用于显示数据总数和当前的数据范围
  size?: string;  // 是大尺寸还是小尺寸显示
  simple?: boolean;  // 是否显示简单分页
  style?: React.CSSProperties;
  locale?: Object;  // 没看到文档中相应的解释
  className?: string;
  prefixCls?: string;
  selectPrefixCls?: string;
  itemRender?: (page: number, type: 'page' | 'prev' | 'next' | 'jump-prev' | 'jump-next') => React.ReactNode;  // 用于自定义页码的结构，可用于优化 SEO，没太理解，看看源码是不是可以理解一下意图
  role?: string;  // 没看到对应的文档解释
}
  ...
  renderPagination = (contextLocale: PaginationLocale) => {
    ...
    return (
      <RcPagination
        ...
        selectComponentClass={isSmall ? MiniSelect : Select}  // 这里用到了Select组件，后面解析时再看
        locale={locale}
      />
    );
  }
...
这里用到了MiniSelect和RcPagination，MiniSelect组件直接复用的Select组件，MiniSelect组件就先不看了，直接看RcPagination组件。
...
export default class Pagination extends React.Component {
  ...
  constructor(props) {
	...
	// current的优先级比defaultCurrent的优先级要高
    let current = props.defaultCurrent;
    if ('current' in props) {
      current = props.current;
    }
	// pageSize的优先级要比defaultPageSize的优先级要高
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if ('pageSize' in nextProps) {
      ...
      current = current > newCurrent ? newCurrent : current;  // 这的逻辑是判断当前页是不是大于调整之后的页面总数，如果大于那么就相当于翻到了最后一页，如果不大于就相当于还是在这页
      if (!('current' in nextProps)) {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.current !== this.state.current && this.paginationNode) {
	  ...
	  // document.activeElement是获取当前的页面获取焦点的dom，如果没有的话可能是返回body或者是null，这是个只读属性
      if (lastCurrentNode && document.activeElement === lastCurrentNode) {
        lastCurrentNode.blur();  // 移除当前元素获得的键盘焦点
      }
    }
  }
  ...
  render() {
    ...
    const goButton = (props.showQuickJumper && props.showQuickJumper.goButton);  // 快速跳转按钮
    const pageBufferSize = props.showLessItems ? 1 : 2;
    const { current, pageSize } = this.state;

    const prevPage = current - 1 > 0 ? current - 1 : 0;
    const nextPage = current + 1 < allPages ? current + 1 : allPages;
	// 去除以data-，aria-，role开始的输入属性
    ...
    // 如果当前页面数量小于某一值时就将所有的页面号码都列举出来，否则有上一页下一页的复杂逻辑展示，不过后面的代码就不看了，逻辑基本上比较清晰了，后面也用了几个其他的组件，也不看了
...
至此Paganition组件就分析完了，其实这个组件的逻辑还是比较清晰的，比Dropdown和Menu组件要简单许多，我们自己写的时候注意一下一些细节就好了，把逻辑理清楚。还有就是这里处理了很多直接敲回车的情况，我们自己做的时候也要注意一下直接敲回车的情况，有的时候确实不会用点击的方法，可能就是直接敲回车比较方便一些。